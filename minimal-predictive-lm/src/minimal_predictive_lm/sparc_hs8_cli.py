...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="SPARC-HS8 sparse episodic and revision conversation shell"
    )
    parser.add_argument("--model", type=Path, help="load an existing HS8 model")
    ...
    model = SPARCHS8Model.load(args.model) if args.model else configured_model()
    if args.prompt is not None:
        print(model.reply(args.prompt).text)
        return

    print("SPARC-HS8 episodic conversation shell")
    print(
        "commands: /read SOURCE PATH, /read-text SOURCE TEXT, "
        "/revise SOURCE TEXT, /stats, /reset, /save PATH, /quit"
    )
    while True:
        try:
        ...
        if user.startswith("/read "):
            ...
            path = Path(path_text)
            inserted = model.ingest(path.read_text(encoding="utf-8"), source_id=source)
            print(f"system> learned {len(inserted)} episodes from {source}")
            continue
        if user.startswith("/read-text "):
            ...
            if len(parts) != 2:
                ...
                continue
            source, text = parts
            inserted = model.ingest(text, source_id=source)
            print(f"system> learned {len(inserted)} episodes from {source}")
            continue
