...
def run_gate(output_dir: str | Path) -> dict[str, object]:
    ...
    long_stream = learner.learn_long_chronological_documents(_long_corpus())
    ...
    long_checks = (
        int(long_stream.documents == 6)
        + int(long_stream.transitions_found == 6)
        + int(len(learner.long_temporal_sources) == 3)
        + int(long_stream.distractor_sentences_ignored >= 8)
        + int(long_stream.maximum_delay >= 3)
        + int(long_stream.ambiguous_transitions == 0)
    )
    axes["long_delayed_event_induction"] = {"correct": long_checks, "total": 6}
    ...
    axes["delayed_event_transfer"] = {"correct": event_correct, "total": 40}
    ...
    continual_text = "記録を開始する。箱Aには5個ある。別の担当者が通り過ぎた。箱Aから3個取り除く。照明が消えた。箱Aには2個ある。"
    learner.learn_long_chronological_document(continual_text, "L7")
    continual_correct = 0
    for index in range(20):
        subject = f"継続箱{index}"
    ...
    ambiguous_before = len(learner.programs)
    ambiguous = learner.learn_long_chronological_document(
        "箱Aには2個ある。箱Aに3個加える。箱Aを3増やす。箱Aには5個ある。", "AMB"
    )
    axes["ambiguous_cause_rejection"] = {
        "correct": 2 if ambiguous.transitions_found == 0 and len(learner.programs) == ambiguous_before else 0,
        "total": 2,
    }
    ...
    axes["independent_fact_relations"] = {
        "correct": 2 if fact_stream.relation_clusters == 2 and kind_reference is not None and location_reference is not None else 0,
        "total": 2,
    }
    ...
    scaffold_keys = [
        ...
        "fixed_three_sentence_layout_supplied", "event_boundaries_supplied", "delay_length_supplied",
    ]
    ...
    report = {
        "stage": "SPARC-highschool-general-001-r7-long-delayed-temporal-passages",
        "axes": axes,
        ...
        "long_temporal_stream": long_stream.__dict__,
        "model": model,
        ...
        "passed": structural,
        "claim_boundary": (
            "Delayed event effects are induced inside long passages with interleaved observations and prose, without event boundaries or delay labels. "
            "The state space remains small and numeric; broad causal narratives, counterfactual reasoning, real textbooks, and open dialogue remain outside this gate."
        ),
    }
    (output / "SPARC-highschool-general-long-temporal.json").write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    (output / "SPARC-highschool-general-long-temporal.model.zlib").write_bytes(learner.to_bytes())
    lines = [
        "# SPARC long delayed-temporal integrated gate", "",
    ...
    (output / "SPARC-highschool-general-long-temporal.md").write_text("\n".join(lines), encoding="utf-8")
    if not structural:
        raise SystemExit("long temporal integrated gate failed")
    return report
