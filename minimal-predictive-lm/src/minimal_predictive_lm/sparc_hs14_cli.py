...
from .sparc_hs14_experiment_v2 import configured_model


def main() -> None:
    parser = argparse.ArgumentParser(
        description="SPARC-HS14 counterexample-guided rule conversation shell"
    )
    parser.add_argument("--model", type=Path, help="load an existing HS14 model")
    ...
    model = SPARCHS14ModelV2.load(args.model) if args.model else configured_model()
    if args.prompt is not None:
        print(model.reply(args.prompt).text)
        return

    print("SPARC-HS14 counterexample-guided conversation shell")
    print(
        "commands: /fact SOURCE TEXT, /revise SOURCE TEXT, /validate PATH, "
