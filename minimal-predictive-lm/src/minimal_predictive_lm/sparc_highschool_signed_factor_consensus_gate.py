...
from .sparc_highschool_evidence_factor_reliability_gate import _strong, _weak, run_gate as run_previous_gate
...
def run_gate(output_dir: str | Path) -> dict[str, object]:
    ...
    for index in range(20):
        ...
        for round_index in range(4):
            target = f"符号校正論点{index}-{round_index}"
            truth = 9 + index + round_index
            add = 2 + round_index % 2
            learner.ingest_factor_verified_hypothesis(
                _strong(target, truth + 7, add, f"誤差系列{round_index}の長文背景と{bad_factor}")
            )
            learner.ingest_factor_verified_hypothesis(
                _strong(target, truth, add, f"独立光学監査{index}-{round_index}による直接再測定")
            )
            learner.ingest_factor_verified_hypothesis(
                _weak(target, truth, add, f"独立重量監査{index}-{round_index}による別系統確認")
            )
            calibration_ok = learner.consolidate_verified_target_factors(target) and calibration_ok

        target = f"未知符号論点{index}"
        truth = 21 + index
        wrong = truth + 8
        add = 3 + index % 2
        learner.ingest_factor_verified_hypothesis(
            _weak(target, truth, add, f"新設観測班{index}による単一点の直接確認")
        )
        learner.ingest_factor_verified_hypothesis(
            _strong(target, wrong, add, f"都市集計資料{index}と{bad_factor}")
        )
        ...
        negative_entries = sum(score < 0 for score in learner._factor_reliability.values())
        passed = (
            calibration_ok
            and learner.factor_consolidations == 4
            and negative_entries > 0
            and resolved.accepted
            and (0, truth) in selected
            and (0, wrong) not in selected
        ...
        examples.append({
            ...
            "negative_factor_entries": negative_entries,
            "factor_reads": f_reads,
            "graph_bytes": graph_bytes,
            "passed": passed,
        })

    axis_name = "signed_verified_source_factor_contradiction_transfer"
    ...
    model.update({
        "shared_bounded_signed_verified_source_factor_consensus": True,
    ...
    report = {
        "stage": "SPARC-highschool-general-001-r25-signed-factor-consensus",
        "baseline": {
            "axes": baseline_axes,
        ...
        "claim_boundary": "The same verified latent-world model now uses both positive and negative bounded source-factor evidence, so repeatedly falsified shared factors can suppress superficially stronger local evidence. Evaluation remains controlled synthetic Japanese; semantic actor/instrument/condition nodes, real textbooks and natural free dialogue remain unproven.",
    }
    (output / "SPARC-highschool-general-signed-factor-consensus.json").write_text(
        json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    (output / "SPARC-highschool-general-signed-factor-consensus.model.zlib").write_bytes(learner.to_bytes())
    (output / "SPARC-highschool-general-signed-factor-consensus.graph.json").write_bytes(
        learner.factor_reliability_graph_bytes()
    )
    if not report["passed"]:
        raise SystemExit("signed verified source factor consensus integrated gate failed")
    return report
