...
from .sparc_highschool_causal_provenance import CausalProvenanceLearner
from .sparc_highschool_document_stream_gate import _corpus
from .sparc_highschool_grounded_explanation_gate import run_gate as run_previous_gate
...
def run_gate(output_dir: str | Path) -> dict[str, object]:
    ...
    for index in range(20):
        subject = f"因果対象{index}"
        initial = 8 + index
        factual_add = 2 + index % 3
        alternative_add = factual_add + 2
        factor = 2
        factual = factor * (initial + factual_add)
        alternative = factor * (initial + alternative_add)
        document = (
            f"{subject}には{initial}個ある。\n"
            f"{subject}に{factual_add}個加える。途中の記録を残した。"
            f"{subject}を{factor}倍にする。{subject}には{factual}個ある。\n"
            f"別の進め方では{subject}に{alternative_add}個加える。"
            "なぜ最終結果に差が生じるのか、途中の変化を根拠に説明せよ。"
        )
        stamp = time.perf_counter()
        result = learner.explain_causal_narrative(document)
        ...
        passed = (
            result.accepted
            and result.verified
            and result.gap_trace == expected_gap
            and result.first_divergence == 1
            and "差が最初に生じた" in result.answer
            and "2倍" in result.answer
    ...
    axis_name = "shared_causal_provenance_explanation"
    ...
    estimated_ops = previous["estimated_sparse_operations"] + max_feature_reads * 60 + learner.paired_state_comparisons
    no_regression = all(
    ...
    report = {
        "stage": "SPARC-highschool-general-001-r12-shared-causal-provenance",
        "baseline": {
            "axes": baseline_axes,
        ...
        "claim_boundary": (
            "The same learner now derives a paired state-difference history from verified factual and counterfactual rollouts, identifies where a difference first appears, and measures how later shared operations propagate it. "
            "The evaluation remains controlled synthetic Japanese; real textbooks, broad causal discourse, implicit world knowledge, and natural open-ended conversation are unproven."
        ),
    }
    (output / "SPARC-highschool-general-causal-provenance.json").write_text(
        json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    (output / "SPARC-highschool-general-causal-provenance.model.zlib").write_bytes(learner.to_bytes())
    lines = [
        "# SPARC shared causal provenance integrated gate",
        "",
    ...
    (output / "SPARC-highschool-general-causal-provenance.md").write_text("\n".join(lines), encoding="utf-8")
    if not report["passed"]:
        raise SystemExit("causal provenance integrated gate failed")
    return report
