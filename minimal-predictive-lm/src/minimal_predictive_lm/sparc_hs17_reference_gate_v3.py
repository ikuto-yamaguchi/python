...
def build_official_middle_holdout() -> object:
    ...
    if not isinstance(rows, list) or len(rows) < OFFICIAL_START + OFFICIAL_COUNT:
        raise ValueError("disambiguation source is unexpectedly small")
    examples = tuple(
        BenchmarkExample(
            f"bbh_disambiguation_middle_{index:03d}",
            "reference_resolution_official_holdout",
            str(item["input"]),
            str(item["target"]),
            "exact",
        )
        for index, item in enumerate(
            rows[OFFICIAL_START : OFFICIAL_START + OFFICIAL_COUNT],
            start=OFFICIAL_START,
        )
    )
    return build_manifest(
        name="sparc-hs17-disambiguation-official-middle",
        split=f"verified-middle-{OFFICIAL_START}-{OFFICIAL_START + OFFICIAL_COUNT - 1}",
...
def run_gate() -> dict[str, object]:
    ...
    official = build_official_middle_holdout()
    policy = base._policy()
    ...
    integrated_report = _run_worker(
        integrated,
        policy,
        model_id="sparc-hs17-v3-integrated",
        timeout=900.0,
    )
    development_report = _run_worker(
        development_tail,
        policy,
        model_id="sparc-hs17-v3-development-tail",
        timeout=300.0,
    )
    official_report = _run_worker(
        official,
        policy,
        model_id="sparc-hs17-v3-official-middle",
        timeout=300.0,
    )
    ...
    development_score = base.score_axis_blind_predictions(
        development_tail, development_report.predictions
    )
    official_score = base.score_axis_blind_predictions(
        official, official_report.predictions
    )
    ...
    development_predictions = {
        row.example_id: row.text for row in development_report.predictions
    }
    official_predictions = {
        row.example_id: row.text for row in official_report.predictions
    }
    ...
    checks = {
        ...
        "development_tail_at_least_28_of_40": development_score.correct >= 28,
        "official_uninspected_middle_at_least_28_of_40": official_score.correct >= 28,
    ...
    return {
        ...
        "disclosure": {
            "cap_gen_first_40_development_status": "inspected",
            "tail_40_development_status": "inspected_after_v2_freeze",
            "official_middle_examples_inspected_before_v3_freeze": 0,
            "official_middle_targets_used_for_training": 0,
            "strict_zero_shot_claim_allowed": False,
            "official_manifest_sha256": official.sha256,
            "official_split": official.split,
        },
        "integrated": {
        ...
        "development_tail": {
            ...
            "failures": base._failure_examples(
                development_tail, development_predictions, per_axis=12
            ),
        },
        "official_uninspected_middle": {
            "score": asdict(official_score),
            "resources": asdict(official_report.resources),
            "failures": base._failure_examples(
                official, official_predictions, per_axis=12
            ),
        },
        "checks": checks,
        "passed": all(checks.values()),
        "claim_boundary": (
            "HS17 V3 uses human-designed English role groundings informed by two "
            "development slices. Its official score is the separately frozen middle "
            "holdout. It is not autonomous or strict-zero-shot reference understanding, "
...
def render_markdown(result: dict[str, object]) -> str:
    ...
    development = result["development_tail"]
    official = result["official_uninspected_middle"]
    score = integrated["score"]
    reference = integrated["axis_scores"]["reference_resolution"]
    return "\n".join(
        [
            "# SPARC-HS17 V3: shared English role compiler",
            "",
            f"Passed: **{result['passed']}**",
            f"- Integrated: **{score['correct']}/{score['examples']} ({100 * score['overall_accuracy']:.2f}%)**",
            f"- Integrated reference: **{reference['correct']}/40**",
            f"- Development tail: **{development['score']['correct']}/40**",
            f"- Official uninspected middle: **{official['score']['correct']}/40**",
