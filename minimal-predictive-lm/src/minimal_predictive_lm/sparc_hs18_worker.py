...
from .english_modifier_order import EnglishModifierOrderResolver
...
def main() -> None:
    ...
    if not source_path:
        raise RuntimeError("MPM_WORDNET_ZIP is required for HS18")
    ...
    modifier_order = EnglishModifierOrderResolver()
    causal = GenericCausalJudgement()
    reference_resolver = EnglishRoleReferenceResolverV5()
    ...
    for example in request["examples"]:
        ...
        if output is None:
            modifier = modifier_order.answer(prompt)
            output = modifier.output
            operations += modifier.operations
            reads += modifier.candidates
            if output is not None:
                writes += 1
                family = "semantic-modifier-order"
        if output is None:
            cause = causal.answer(prompt)
            output = cause.output
            operations += cause.operations
            reads += cause.evidence
            if output is not None:
                writes += 1
                family = "generic-causal-model"
        if output is None:
    ...
    program_bits = (
        ...
        + modifier_order.description_bits
        + causal.description_bits
    ...
    json.dump(
        {
            ...
            "metadata": {
                ...
                "modifier_runtime_bits": modifier_order.description_bits,
                "causal_runtime_bits": causal.description_bits,
