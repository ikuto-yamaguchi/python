...
def main() -> None:
    ...
    causal_resolver = EnglishCausalResolver()
    ...
    for example in request["examples"]:
        ...
        if output is None:
            causal = causal_resolver.answer(prompt)
            output = causal.output
            operations += causal.operations
            reads += 1 if causal.output is not None else 0
            if output is not None:
                writes += 1
                family = f"shared-event-causal:{causal.family}"
        if output is None:
    ...
    program_bits = (
        ...
        + causal_resolver.description_bits
    ...
    json.dump(
        {
            ...
            "metadata": {
                ...
                "causal_runtime_bits": causal_resolver.description_bits,
                "reference_compiler_version": 5,
                "causal_compiler_version": 1,
                "benchmark_task_name_branches": 0,
                "domain_specific_handlers": 0,
                "public_targets_used_for_causal_rules": 0,
            },
        },
        sys.stdout,
        ensure_ascii=False,
