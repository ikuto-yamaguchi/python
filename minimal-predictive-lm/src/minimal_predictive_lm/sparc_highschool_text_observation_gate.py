...
def run_gate(output_dir: str | Path) -> dict[str, object]:
    ...
    for index in range(30):
        ...
        fact_cases.append((surfaces[index % len(surfaces)](subject, obj), subject, obj))
    correct = 0
    induced_relations = set()
    for text, subject, obj in fact_cases:
    ...
    axes["raw_fact_relation_induction"] = {"correct": correct, "total": len(fact_cases)}

    observation_cases = []
    for index in range(20, 40):
        observation_cases.append((f"箱{index}には{index}個ある", f"箱{index}", count_relation, index))
        observation_cases.append((f"試料{index}の温度は{index}度である", f"試料{index}", temperature_relation, index))
    correct = 0
    for text, subject, relation, value in observation_cases:
        query_started = time.perf_counter()
    ...
    axes["raw_numeric_world_induction"] = {"correct": correct, "total": len(observation_cases)}

    event_cases = []
    for index in range(10, 30):
        event_cases.append((f"箱{index}に3個加える", f"箱{index}", index, index + 3))
        event_cases.append((f"箱{index}を2倍にする", f"箱{index}", index, index * 2))
    correct = 0
    for text, subject, old, expected in event_cases:
        before = learner.observe_world([f"{subject}には{old}個ある"]).world
        result = apply(text, before)
        correct += int(result.accepted and result.world.number_map().get((subject, count_relation)) == expected)
    axes["text_only_event_program_transfer"] = {"correct": correct, "total": len(event_cases)}
    ...
    for index in range(20):
        ...
        causal_correct += int(second.accepted and second.world.number_map().get((subject, temperature_relation)) == 20 + index)
    ...
    for index in range(20):
        ...
        continual_correct += int(
            removed.accepted
            and removed.world.number_map().get((subject, count_relation)) == index + 2
            and retained.accepted
            and retained.world.number_map().get((subject, count_relation)) == index + 8
        )
    ...
    for text, subject, obj in paraphrases:
        result = apply(text, World())
        paraphrase_correct += int(result.accepted and any(f[0] == subject and f[2] == obj for f in result.world.facts))
    axes["untouched_paraphrase_transfer"] = {"correct": paraphrase_correct, "total": len(paraphrases)}

    explanation_correct = 0
    explanation_samples = []
    relation = next(iter(induced_relations)) if len(induced_relations) == 1 else ""
    ...
    for text in ["今日は静かな雨が降る", "この意見を詳しく論証せよ", "未知の規則で変換する"] * 10:
        ...
        unknown_correct += int(not result.accepted and result.world == before and observation.abstained == 1)
    ...
    for index in range(20):
        ...
        dialogue_correct += int(world.number_map().get((subject, count_relation)) == 106)
    axes["bounded_long_dialogue_state"] = {"correct": dialogue_correct, "total": 20}

    report_model = learner.report()
    caller_scaffolds = int(not report_model["relation_ids_supplied_by_caller"]) + int(not report_model["grounded_worlds_supplied_by_caller"])
    ...
    structural = (
        all(percentages[name] >= 0.95 for name in [
            "raw_fact_relation_induction", "raw_numeric_world_induction",
            "text_only_event_program_transfer", "causal_state_simulation",
            "goal_directed_planning", "continual_program_growth",
            "free_form_explanation", "unknown_abstention",
            "bounded_long_dialogue_state", "caller_scaffolds_removed",
        ])
        and percentages["untouched_paraphrase_transfer"] >= 0.50
        and report_model["serialized_bytes"] <= 65536
        and peak_rss <= 536870912
        and wall <= 5.0
    )
    highschool = (
        total_correct / total >= 0.90
        and min(percentages.values()) >= 0.75
        and False  # real textbook, open dialogue, and broad curriculum gates are not yet present
    )
    report = {
        ...
        "model": report_model,
        "opaque_fact_relations": sorted(induced_relations),
        ...
        "passed": structural,
        "claim_boundary": (
            "The caller no longer supplies relation IDs or World objects for the new text-observation and transition-training path. "
            "Paraphrase groups still provide repeated co-denotation, the corpus is generated, and unrestricted textbook reading and open dialogue remain untested."
        ),
    }
    (output / "SPARC-highschool-general-text-observation.json").write_text(
    ...
    lines = [
        ...
        f"- model bytes: {report_model['serialized_bytes']}",
        f"- peak RSS: {peak_rss}",
