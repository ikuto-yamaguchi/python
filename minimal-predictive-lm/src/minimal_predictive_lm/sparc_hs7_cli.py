...
from .sparc_hs7_experiment import configured_model
from .sparc_reasoning import ReasoningResult
from .sparc_units import SPARCHS7Model


def main() -> None:
    parser = argparse.ArgumentParser(description="SPARC-HS7 unit-typed model")
    parser.add_argument("--model", type=Path)
    parser.add_argument("--prompt")
    parser.add_argument("--save", type=Path)
    args = parser.parse_args()

    model = SPARCHS7Model.load(args.model) if args.model else configured_model()
    if args.prompt is not None:
        print(model.reply(args.prompt).text)
        return

    print("SPARC-HS7 unit-typed conversation shell")
    print("commands: /teach-units FILE.json, /stats, /save PATH, /quit")
    while True:
        try:
        ...
        if user.startswith("/save "):
            ...
            continue
        if user.startswith("/teach-units "):
            path = Path(user[13:].strip())
            rows = json.loads(path.read_text(encoding="utf-8"))
            examples = [(str(row["problem"]), row["answer"]) for row in rows]
            expression = model.teach_units(examples, max_cost=9)
            print(
                f"system> synthesized typed plan; cost={expression.cost}; "
                f"states={expression.semantic_states}; output-dimension={expression.output_dimension}"
            )
            continue
