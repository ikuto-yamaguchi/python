...
def unmarked_spans(s,cap=6):
    # ontology-free local rarity/length proposals; intentionally weak.
    spans=[]
    for n in (2,3,4,5):
    ...
    # deterministic diversity rather than semantic dictionary
...
def make_example(rng,mode):
    ...
    # Candidate graph is composed from raw spans and generic binary edge options.
    # Caps keep weak-CPU exploration bounded.
    ...
    if marked and len(spans)>=2:
        # evaluator-only mapping: first two distinct raw quotes; plan/nested scope differs.
        truth=Candidate(0,1,1,1 if mode=='plan' else 0,1)
        if truth not in cands:
            if len(cands)>=16:cands[-1]=truth
            else:cands.append(truth)
    # Environment outcomes are generic binary contradictions per candidate.
    causes={c:[] for c in CAUSE_NAMES}
    for i,c in enumerate(cands):
        if truth is None:
            # candidate-collapse / proposal-missing: all candidates incur mixed residual.
        ...
        # Sparse observation noise: delayed/generic feedback is imperfect.
        # Remove some real contradictions and inject a few spurious ones.
        for cause in CAUSE_NAMES:
            kept=[]
    ...
    def solve(self,ex):
        ...
        for sweep in range(1,self.max_sweeps+1):
            for cause,bad in ex.causes.items():
                ...
                for edge in edges:
                    # Cause-edge bipartite routing pools contradiction evidence over
                    # candidates sharing the same local edge value. This is the
                    # denoising/credit-routing mechanism under test.
                    groups={}
                    for i,cand in enumerate(ex.candidates):
                        val=getattr(cand,edge)
                        groups.setdefault(val,[]).append(i)
                    rates={}
                    for val,ids in groups.items():
                        rates[val]=sum(i in badset for i in ids)/max(1,len(ids))
                    for i,cand in enumerate(ex.candidates):
            ...
            prev=state
        totals=[sum(edge_energy[i].values()) for i in range(H)]
        best=min(totals); residual=sum(1 for cause in CAUSE_NAMES if ex.causes[cause])
        # null-cause holds unexplained contradiction rather than forcing a candidate.
        null=self.null_cause and (ex.truth is None and residual>=3)
        pred=None if null or len(active)!=1 else next(iter(active))
        return {'pred':pred,'sweeps':sweep,'active':len(active),'converged':converged,'flat':len(active)>1,'null':null,'energy':best}

    def local_update(self,ex,res):
        # phase-like local update: only cause-edge pairs that exclude wrong candidates
        # without touching truth receive small support. Evaluator truth is used solely
        # to score the controlled learning rule, not to generate candidate labels.
...
def run(seed,n,mode,method):
    ...
    examples=[make_example(rng,mode) for _ in range(n)]
    for ex in examples:
    ...
    return {
      ...
      'candidate_mean':statistics.mean(len(e.candidates) for e in examples),
      'unrouted':model.unrouted,
    }
