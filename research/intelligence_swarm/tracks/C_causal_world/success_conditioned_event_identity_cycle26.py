...
class Event:
    ...
    env_success:tuple=(); env_wrong:tuple=(); signature:tuple=(); cls:int=-1
...
class Model:
    ...
    def factor_signature(self,e,rows):
        success=[]
        for x in rows:
            pred,ok,amb=self.apply(x.before,x.command,e)
            if ok and amb==1 and pred==x.after:success.append(x)
        if len(success)<2:return None
        comps=[]
        for x in success[:8]:
            base,_,_=self.apply(x.before,x.command,e)
            cmd_parts=[x.command[i:j] for i in range(len(x.command)) for j in range(i+1,min(len(x.command),i+11)+1)]
            obj=[q for q in cmd_parts if len(q)>=2 and q in x.before and q in x.after and q in x.future]
            val=[q for q in cmd_parts if q in x.after and q in x.future and q not in x.before]
            oc=min(obj,key=len) if obj else ''
            vc=min(val,key=len) if val else ''
            y=success[(success.index(x)+1)%len(success)]
            obj2='';val2=''
            yp=[y.command[i:j] for i in range(len(y.command)) for j in range(i+1,min(len(y.command),i+11)+1)]
            yo=[q for q in yp if len(q)>=2 and q in y.before and q in y.after and q in y.future]
            yv=[q for q in yp if q in y.after and q in y.future and q not in y.before]
            if yo:obj2=min(yo,key=len)
            if yv:val2=min(yv,key=len)
            variants=[]
            for name,a,b in [('object',oc,obj2),('value',vc,val2),('context',e.sl,e.sr[::-1])]:
                if not a or not b or a==b:
                    variants.append((name,0,0,0));continue
                bc=x.before.replace(a,b,1);cc=x.command.replace(a,b,1)
                pp,ok,amb=self.apply(bc,cc,e)
                target=int(ok and amb==1 and pp!=base)
                damage=int(ok and '補助記録' not in pp)
                future=int(ok and any(z in x.future for z in (vc,val2) if z))
                variants.append((name,target,damage,future))
            comps.append(tuple(variants))
        flat=[]
        for k in range(3):
            flat.append(tuple(round(statistics.mean(c[k][j] for c in comps),2) for j in range(1,4)))
        return tuple(flat)
    def fit(self,train):
        ...
        for x in train:
            ...
            if p<0:continue
            e=Event(x.command[max(0,p-8):p],x.command[p+len(new):p+len(new)+8],x.before[max(0,l-8):l],x.before[len(x.before)-r:len(x.before)-r+8] if r else '',shape(old),shape(new))
        ...
        for i,e in enumerate(self.events):
            ss=[];ww=[]
            for env in envs:
                rows=[x for x in train if x.env==env];s=w=0
                for x in rows:
                    p,ok,a=self.apply(x.before,x.command,e)
                    if ok and a==1:
                        if p==x.after:s+=1
                        else:w+=1
                ss.append(s);ww.append(w)
            e.env_success=tuple(ss);e.env_wrong=tuple(ww)
            if sum(v>=2 for v in ss)>=2 and sum(ww)==0:
                sig=self.factor_signature(e,train)
                if sig is not None:e.signature=sig;eligible.append(i)
        if self.kind in ('identity','graph'):
            buckets=defaultdict(list)
            for i in eligible:buckets[self.events[i].signature].append(i)
            for ids in buckets.values():
                if len(ids)>=2:
                    cid=len(self.classes);self.classes.append(tuple(ids))
                    for i in ids:self.events[i].cls=cid
        if self.kind=='graph':
            for a,ca in enumerate(self.classes):
                for b,cb in enumerate(self.classes):
                    if a==b:continue
                    ea=self.events[ca[0]];eb=self.events[cb[0]]
                    if ea.newshape==eb.oldshape:self.edges.append((a,b))
            self.edges=self.edges[:96]
    ...
    def predict(self,x):
        ...
        for e in self.events:
            ...
            if self.kind in ('identity','graph'):
                score+=.2*(e.cls>=0)+.05*sum(e.env_success)
            cand.append((score,p))
...
def evaluate(seed,n,mode):
    ...
    for kind in ('surface','success','graph'):
        M=Model(kind if kind!='success' else 'identity');M.fit(train);t=time.perf_counter();c=w=z=cs=0
        for x in test:
            p,k,nul=M.predict(x);cs+=k;c+=int(p==x.after);w+=int(p!=x.after and not nul);z+=nul
        eligible=sum(bool(e.signature) for e in M.events)
        out[kind]={'accuracy':c/len(test),'wrong_commit':w/len(test),'null_rate':z/len(test),'mean_candidates':cs/len(test),'events':len(M.events),'eligible_events':eligible,'identity_classes':len(M.classes),'identity_members':sum(len(q) for q in M.classes),'edges':len(M.edges),'model_bytes':len(pickle.dumps(M)),'training_seconds':M.train_seconds,'inference_ms':(time.perf_counter()-t)*1000/len(test)}
    return out

def sequential(seed,n):
    seq=build(seed+700,n,'seen',0);out={}
    for kind in ('surface','success','graph'):
        M=Model(kind if kind!='success' else 'identity');M.fit(seq[:n//2]);cov=cor=0
        for a,b in zip(seq[n//2:-1],seq[n//2+1:]):
            p1,c1,z1=M.predict(a);b2=Ex(p1,b.command,b.after,b.future,b.env,b.obj,b.field,b.old,b.new,b.focus);p2,c2,z2=M.predict(b2)
...
def main():
    ...
    for n,runs in raw.items():
        summary[n]={}
        for m in modes:
            summary[n][m]={k:{x:statistics.mean(r[m][k][x] for r in runs) for x in runs[0][m][k]} for k in ('surface','success','graph')}
        summary[n]['sequential']={k:{x:statistics.mean(r['sequential'][k][x] for r in runs) for x in runs[0]['sequential'][k]} for k in ('surface','success','graph')}
    payload={'hypothesis':'Success-Conditioned Event Identity from Factorized Intervention Target Signatures','seeds':[1,7,19],'sizes':[48,144,288],'raw':raw,'summary':summary,'peak_rss_kib_runtime_included':resource.getrusage(resource.RUSAGE_SELF).ru_maxrss,'estimated_complexity':'event extraction O(NL), positive replay O(PNE), factor swaps O(PSF), partition O(P), graph O(C^2), inference O(PL)','hidden_labels_used_by_learner':False,'highschool_level_passed':False,'native_japanese_communication_passed':False,'weak_smartphone_verified':False,'completion':False}
