...
def endpoint_apply(before,value,p):
    starts=[];q=0
    while True:
    ...
    a,b=starts[0];return before[:a]+value+before[b:]

def feature_set(e,o,v,p):
    feats=set()
    for src,name in ((e.command,'c'),(e.before,'b'),(e.future,'f')):
        for n in (1,2,3,4):
            if len(src)>=n:
                feats.add(name+'^'+src[:n]);feats.add(name+'$'+src[-n:])
    feats.add('olen:'+str(len(o)));feats.add('vlen:'+str(len(v)))
    feats.add('osh:'+shape(o));feats.add('vsh:'+shape(v))
    feats.add('epL:'+p.sl[-4:]);feats.add('epR:'+p.sr[:4])
    feats.add('o_cmd:'+str(o in e.command));feats.add('o_state:'+str(o in e.before))
    feats.add('v_cmd:'+str(v in e.command));feats.add('v_future:'+str(v in e.future))
    return feats

class Model:
    def __init__(self,mode):
        self.mode=mode;self.endpoints=[];self.triangles=[]
        self.raw_candidates=0;self.audit_count=0;self.cut_count=0
        self.training_seconds=0.;self.description_bits=0
    def fit(self,eps):
        ...
        for e in eps:
            ...
            ep=(sl,sr,shape(old),shape(new));epc[ep]+=1
            oc=[x for x in spans(e.command)&spans(e.before)&spans(e.after)&spans(e.future) if 2<=len(x)<=12]
            vc=[x for x in spans(e.command)&spans(e.after)&spans(e.future) if x not in e.before and 1<=len(x)<=10]
            oc=sorted(oc,key=lambda x:(-len(x),x))[:4]
            vc=sorted(vc,key=lambda x:(-len(x),x))[:4]
            self.raw_candidates+=len(oc)+len(vc);records.append((e,ep,oc,vc))
        ...
        stats=defaultdict(lambda:[0,0,0,defaultdict(lambda:[0,0])])
        for e,ep,ocs,vcs in records:
            ...
            for o in ocs:
                for v in vcs:
                    self.audit_count+=1;pred=endpoint_apply(e.before,v,p)
                    key=(shape(o),shape(v),pi)
                    ok=(pred==e.after and o in e.command and o in e.before)
                    if pred is None:stats[key][2]+=1
                    elif ok:stats[key][0]+=1
                    else:stats[key][1]+=1
                    for feat in feature_set(e,o,v,p):
                        stats[key][3][feat][0]+=int(ok)
                        stats[key][3][feat][1]+=int(not ok)
        for (osh,vsh,pi),(pos,wrong,noexec,fc) in stats.items():
            if pos<3 or wrong>pos:continue
            cuts=[]
            for feat,(pp,ww) in fc.items():
                if pp>=max(2,math.ceil(.6*pos)) and ww<=max(0,math.floor(.2*max(1,wrong))):
                    gain=pp-ww-.05*len(feat);cuts.append((gain,feat))
            cuts=tuple(x for _,x in sorted(cuts,reverse=True)[:4])
            tri=Triangle(osh,vsh,pi,pos,wrong,noexec,cuts,96+sum(8*len(x) for x in cuts))
            self.triangles.append(tri);self.cut_count+=len(cuts)
        self.triangles=sorted(self.triangles,key=lambda t:(t.support-t.wrong,len(t.cuts)),reverse=True)[:64]
    ...
    def predict(self,e):
        ...
        for t in self.triangles:
            ...
            for o in os:
                for v in vs:
                    pred=endpoint_apply(e.before,v,p)
                    if pred is None:continue
                    feats=feature_set(e,o,v,p)
                    hit=sum(c in feats for c in t.cuts)
                    score=t.support-1.5*t.wrong+2*hit-.25*(len(t.cuts)-hit)
                    if self.mode=='graph':score=t.support-t.wrong
                    cand.append((score,pred));pairs.append((score,o,v))
        if not cand:return e.before,False,0,[]
        by={}
        for score,pred in cand:by[pred]=max(score,by.get(pred,-1e9))
        ranked=sorted(((s,p) for p,s in by.items()),reverse=True)
        if len(ranked)>1 and ranked[0][0]-ranked[1][0]<.5:
            return e.before,False,len(ranked),pairs[:32]
        return ranked[0][1],True,len(ranked),pairs[:32]

def evaluate(model,test):
    t=time.perf_counter();correct=wrong=commit=recall=cands=0
    for e in test:
        pred,did,n,pairs=model.predict(e);cands+=n;commit+=did
        correct+=int(did and pred==e.after);wrong+=int(did and pred!=e.after)
        recall+=int(any(o==e.obj and v==e.new for _,o,v in pairs))
    return {'accuracy':correct/len(test),'wrong_commit':wrong/len(test),'commit_rate':commit/len(test),'pair_recall':recall/len(test),'mean_candidates':cands/len(test),'triangles':len(model.triangles),'cuts':model.cut_count,'raw_candidates':model.raw_candidates,'audit_count':model.audit_count,'description_bits':model.description_bits,'model_bytes':len(pickle.dumps(model)),'training_seconds':model.training_seconds,'inference_ms':(time.perf_counter()-t)*1000/len(test)}

def main():
    ap=argparse.ArgumentParser();ap.add_argument('--output',default='MEASUREMENTS_CYCLE_027.json');a=ap.parse_args()
    modes=['seen','order','lexeme','rename','alternate','nested','omitted','paragraph'];raw={}
    for n in (48,144,288):
        runs=[]
        for seed in (1,7,19):
            train=build(seed,n,'seen');models={}
            for mode in ('graph','cut','mdl'):
                m=Model(mode);m.fit(train);models[mode]=m
            run={}
            for mode in modes:
                test=build(seed+999,24,mode)
    ...
    for n,runs in raw.items():
        ...
        for mode in modes:
            summary[n][mode]={}
            for method in ('graph','cut','mdl'):
                summary[n][mode][method]={k:statistics.mean(r[mode][method][k] for r in runs) for k in runs[0][mode][method]}
    payload={'hypothesis':'Contrastive Binding Graph Selection by Minimal Negative Witness Cuts','seeds':[1,7,19],'sizes':[48,144,288],'raw':raw,'summary':summary,'peak_rss_kib_runtime_included':resource.getrusage(resource.RUSAGE_SELF).ru_maxrss,'estimated_complexity':'proposal O(NL^2), witness audit O(NK_oK_v), cut induction O(TF), inference O(TL^2), T<=64','hidden_labels_used_by_learner':False,'highschool_level_passed':False,'native_japanese_communication_passed':False,'weak_smartphone_verified':False,'completion':False}
